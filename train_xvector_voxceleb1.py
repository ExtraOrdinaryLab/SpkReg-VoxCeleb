...
from models.xvector.logging import ColoredFormatter
from models.xvector.configuration_xvector import XVectorConfig
from models.xvector.feature_extraction_xvector import XVectorFeatureExtractor
from models.xvector.modeling_xvector import XVectorForSequenceClassification
...
def main():
    ...
    send_example_telemetry("run_xvector_voxceleb", model_args, data_args)
    ...
    del raw_datasets['test'] # only need voxceleb dev set

    if os.path.isfile('voxceleb1.spk'):
        logger.info('voxceleb1.spk file found, skipping speaker label creation.')
        speakers = [line.strip() for line in open('voxceleb1.spk', 'r').readlines()]
        speakers = list(sorted(set(speakers)))
    else:
        speakers = [example[data_args.label_column_name] for example in tqdm(raw_datasets['train'])]
        speakers = list(sorted(set(speakers)))
        with open('voxceleb1.spk', 'w') as f:
            for speaker in speakers:
                f.write(f"{speaker}\n")
    raw_datasets = raw_datasets.cast_column(
        data_args.label_column_name, ClassLabel(num_classes=len(speakers), names=speakers)
    )
    ...
    feature_extractor = XVectorFeatureExtractor(
        feature_size=1,
        sampling_rate=16000,
        padding_value=0,
    ...
    config = XVectorConfig.from_pretrained(
        model_args.config_name, 
        label2id=label2id,
        id2label=id2label, 
    )
    model = XVectorForSequenceClassification(config)
